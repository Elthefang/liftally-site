...
async function getClient() {
  if (!clientPromise) {
    clientPromise = import('./firebase-client.js').then((module) => {
...
function assertProduction(client) {
  if (client.environment !== 'production' || client.app.options.projectId !== 'myappproject-f71fd') {
    throw new Error('Firebase production backend is not configured for myappproject-f71fd.');
  }
}
...
async function createAccountWithEmail(email, password) {
  ...
  const userCredential = existingUser && existingUser.isAnonymous
    ? await authSdk.linkWithCredential(existingUser, credential)
    : await authSdk.createUserWithEmailAndPassword(client.auth, email, password);
...
async function signInWithEmail(email, password) {
  const client = await getClient();
  assertProduction(client);
  const credential = await client.sdk.auth.signInWithEmailAndPassword(client.auth, email, password);
